...
def preprocess_observations(input_observation, prev_processed_observation, input_dimensions):
    ...
    processed_observation = processed_observation.astype(np.float).ravel()  # Convert from 80 x 80 matrix to 1600 x 1 matrix
...
def main():
    ...
    # hyperparameters
    episode_number = 0 
    batch_size = 3 # How many rounds we play before update weights of our network
    gamma = 0.99 # Discount factor for reward
    decay_rate = 0.99 # Parameter used in RMSProg algorithm
    num_hidden_layer_neurons = 200 # 1 hidden layer - 200 neurons 
    input_dimensions = 80 * 80 # Dimension of our observation image
    learning_rate = 1e-3 # higher rate means we react more to results and a lower rate means we do not react as strongly to each result. 
    ...
    while True:
        ...
        if done: # an episode finished
            ...
            print '%f: resetting env. episode reward total was %f. running mean: %f' % (episode_number, reward_sum, running_reward)
            reward_sum = 0
            prev_processed_observations = None

            # if running_reward > 0 
            #     print 'yeeeeeee, episode number = %f' % (episode_number)
